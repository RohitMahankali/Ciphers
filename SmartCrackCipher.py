...
def main():
    ...
    kList = kText.split()
    for item in kList:
        for key in kBigrams.keys():
            if key in item:
                kBigrams[key] += 1

    for key in kBigrams.keys():
        kBigrams[key] /= (length_known * 1.0)
    ...
    cList = cText.split()
    for item in cList:
        for key in cBigrams.keys():
            if key in item:
                cBigrams[key]+=1

    for key in cBigrams.keys():
        cBigrams[key] /= (length_cipher * 1.0)
    ...
    for key in kBigrams.keys():
        li3.append(key)
        li3.sort(key=lambda k: kBigrams[k])

    for key in cBigrams.keys():
        li4.append(key)
        li4.sort(key=lambda k: cBigrams[k])
    ...
    for i in range(len(li4)):
        ...
        kFirst = li3[i][0]
        kSecond = li3[i][1]
        kLetters[kFirst] += kBigrams[li3[i]]
        cLetters[first] += cBigrams[li4[i]]
        kLetters[kSecond] += kBigrams[li3[i]]
        cLetters[second] += cBigrams[li4[i]]
